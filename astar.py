...
class Point:
    ...
    def neighbor_update(self, grid):
        ...
        self.neighbors = []
        if self.row < grid.total_row - 1 and not grid[self.row+1][self.col].is_barrier(): #down
            self.neighbors.append(grid[self.row+1][self.col])

        if self.row >0 and not grid[self.row-1][self.col].is_barrier(): #up
            self.neighbors.append(grid[self.row-1][self.col])
        
        if self.row < grid.total_row - 1 and not grid[self.row][self.col+1].is_barrier(): #right
...
def make_grid(row, width):
    ...
    for i in range(row):
        grid.append([])
        for j in range(row):
            grid[i].append(Point(i, j, gap, gap))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key== pygame.K_SPACE and not finding:
                    ...
                    algorithm(lambda: plot(plot, grid, ROWS, width), grid, start, end)
